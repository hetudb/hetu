...
use hetu_core::error::{BallistaError, Result};
use hetu_core::event_loop::{EventAction, EventSender};
use hetu_core::execution_plans::UnresolvedShuffleExec;
...
use hetu_core::serde::scheduler::{ExecutorMetadata, PartitionStats};
use hetu_core::serde::{protobuf, AsExecutionPlan};
        ...
        // All of the unresolved shuffles are completed, update the stage plan
        {
            let mut partition_locations: HashMap<
                ...
                HashMap<
                    ...
                    Vec<hetu_core::serde::scheduler::PartitionLocation>, // shuffle partitions
                >,
            > = HashMap::new();
                            ...
                            Some(task_status::Status::Completed(CompletedTask {
                                ...
                                for shuffle_write_partition in partitions {
                                    ...
                                    let partition_location =
                                        hetu_core::serde::scheduler::PartitionLocation {
                                            partition_id:
                                                hetu_core::serde::scheduler::PartitionId {
