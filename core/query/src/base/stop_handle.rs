...
use crate::base::stoppable::Stoppable;
use futures::Future;
...
impl StopHandle {
    ...
    ) -> Result<impl Future<Output = ()> + Send + '_, HetuError> {
        if self
            .stopping
            .compare_exchange(false, true, Ordering::Relaxed, Ordering::Relaxed)
            .is_err()
        {
            return Err(HetuError::Internal(String::from(
                "StopHandle is shutting down",
            )));
        }
    ...
    pub fn wait_to_terminate(
        ...
        async move {
            ...
            println!("Press Ctrl + C again to force shutdown.");
    ...
    pub fn install_termination_handle() -> broadcast::Sender<()> {
        ...
        ctrlc::set_handler(move || {
            if let Err(error) = t.send(()) {
                println!("Could not send signal on channel {}", error);
                std::process::exit(1);
            }
        })
