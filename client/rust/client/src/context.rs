...
use hetu_core::config::BallistaConfig;
use hetu_core::serde::protobuf::scheduler_grpc_client::SchedulerGrpcClient;
use hetu_core::serde::protobuf::{ExecuteQueryParams, KeyValuePair};
use hetu_core::utils::create_df_ctx_with_ballista_query_planner;
...
pub struct HetuContext {
    state: Arc<Mutex<HetuContextState>>,
    context: Arc<SessionContext>,
}

impl HetuContext {
    /// Create a context for executing queries against a remote Hetu scheduler instance
    pub async fn remote(
        host: &str,
        port: u16,
        config: &BallistaConfig,
    ) -> hetu_core::error::Result<Self> {
        let state = HetuContextState::new(host.to_owned(), port, config);

        let scheduler_url =
            format!("http://{}:{}", &state.scheduler_host, state.scheduler_port);
        info!("Connecting to Hetu scheduler at {}", scheduler_url.clone());
    ...
    pub async fn standalone(
        ...
        use hetu_core::serde::protobuf::PhysicalPlanNode;
        use hetu_core::serde::BallistaCodec;

        log::info!("Running in local mode. Scheduler will be run in-proc");

        let addr = hetu_cloudsrv::standalone::new_standalone_scheduler().await?;
        ...
        hetu_query::new_standalone_executor(scheduler, concurrent_tasks, default_codec)
            .await?;

        let state = HetuContextState::new("localhost".to_string(), addr.port(), config);
