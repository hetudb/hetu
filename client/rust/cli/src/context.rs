...
/// The CLI supports using a local DataFusion context or a distributed HetuContext
pub enum Context {
...
impl Context {
    ...
    pub async fn new_remote(host: &str, port: u16) -> Result<Context> {
        Ok(Context::Remote(HetuContext::try_new(host, port).await?))
    }
...
// implement wrappers around the HetuContext to support running without hetu

// #[cfg(feature = "hetu-client")]
pub struct HetuContext(hetu_client::context::HetuContext);
// #[cfg(feature = "hetu-client")]
impl HetuContext {
    pub async fn try_new(host: &str, port: u16) -> Result<Self> {
        use hetu_client::context::HetuContext;
        use hetu_client::prelude::BallistaConfig;
        let builder =
            BallistaConfig::builder().set("ballista.with_information_schema", "true");
        let config = builder
            .build()
            .map_err(|e| DataFusionError::Execution(format!("{:?}", e)))?;
        let remote_ctx = HetuContext::remote(host, port, &config)
            .await
            .map_err(|e| DataFusionError::Execution(format!("{:?}", e)))?;
        Ok(Self(remote_ctx))
    }
...
// #[cfg(not(feature = "hetu-client"))]
// pub struct HetuContext();
// #[cfg(not(feature = "hetu-client"))]
// impl HetuContext {
//     pub async fn try_new(_host: &str, _port: u16) -> Result<Self> {
//         Err(DataFusionError::NotImplemented(
//             "Remote execution not supported. Compile with feature 'hetu-client' to enable"
//                 .to_string(),
//         ))
//     }
